...
export default function MouthSection() {
  return (
    <div className="space-y-10">
      {/* Tone spectrum */}
      <div>
        <p className="text-xs uppercase tracking-[0.2em] text-rc-warm/40 mb-6">The spectrum</p>
        <div className="space-y-3">
          {[
            ...
            <div key={label} className="flex items-center gap-3">
              <span className="text-[10px] text-rc-warm/40 w-16 text-right">{left}</span>
              ...
              <span className="text-[10px] text-rc-warm/40 w-16">{right}</span>
              <span className="text-[10px] text-rc-bark/60 font-medium w-16">{label}</span>
            </div>
          ))}
        </div>
      </div>
      ...
      <div className="grid grid-cols-2 gap-4">
        <div className="bg-white rounded-2xl border border-rc-fog p-5">
          <p className="text-xs uppercase tracking-[0.2em] text-rc-sage font-semibold mb-3">Words we use</p>
          <div className="flex flex-wrap gap-1.5">
            {WORDS_WE_USE.map((w) => (
              <span key={w} className="px-2.5 py-1 text-[11px] font-medium text-rc-sage bg-rc-sage/8 rounded-full">{w}</span>
            ))}
          </div>
        </div>
        <div className="bg-white rounded-2xl border border-rc-fog p-5">
          <p className="text-xs uppercase tracking-[0.2em] text-rc-reject font-semibold mb-3">Never use</p>
          <div className="flex flex-wrap gap-1.5">
            {WORDS_WE_NEVER_USE.map((w) => (
              <span key={w} className="px-2.5 py-1 text-[11px] font-medium text-rc-reject/60 bg-rc-reject/5 rounded-full">{w}</span>
            ))}
          </div>
        </div>
      </div>
